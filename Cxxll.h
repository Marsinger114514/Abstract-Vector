...
class Cxxll {
public:
    Cxxll() : head(nullptr), size(0), capacity(1024) {}    // 构造函数

    Cxxll(const Cxxll<T>& other) : size(other.size), capacity(other.capacity)     //深拷贝构造
    {
        head = new T[capacity];
    ...
    bool operator==(const Cxxll<T>& v) const;    // 判断本向量和v是否相同
    bool operator!=(const Cxxll<T>& v) const;	//判断本向量和v是否不同
...
private:
    ...
    template <typename T>
    T& Cxxll<T>:: operator[](const unsigned int& pos) const
    {
        if (pos < 0 || pos >= size) {
            throw -1;
        }
    ...
    template <typename T>
    bool Cxxll<T>:: operator==(const Cxxll<T>& v) const
    {
        if (size != v.size) {
            return false;
        }
    ...
    template <typename T>
    unsigned int Cxxll<T>:: getcapacity() const
    {
        return capacity;
    }

    template <typename T>
    void Cxxll<T>:: insert(const unsigned int& pos, const T& val, const unsigned int& n)
    {
        if (pos < 0 || pos > size) {
            throw -1;
        }
    ...
    template <typename T>
    void Cxxll<T>:: push_back(const T &val)
    {
        if (size == capacity)
        {
            resize();
    ...
    template <typename T>
    T& Cxxll<T>:: at(const unsigned int& pos)
    {
        if (pos < 0 || pos >= size)
        {
            throw -1;
